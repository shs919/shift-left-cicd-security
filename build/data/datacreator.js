...
async function createChallenges() {
    ...
    await Promise.all(challenges.map(async ({ name, category, description, difficulty, hints, mitigationUrl, key, disabledEnv, tutorial, tags }) => {
...
async function createHints(ChallengeId, hints) {
    let i = 0;
    return await Promise.all(hints.map(async (hint) => {
        hint = hint.replace(/OWASP Juice Shop/, `${config_1.default.get('application.name')}`);
        return await hint_1.HintModel.create({
            ChallengeId,
            text: hint,
            order: ++i,
            unlocked: false
        }).catch((err) => {
            logger_1.default.error(`Could not create hint: ${utils.getErrorMessage(err)}`);
        });
    }));
}
