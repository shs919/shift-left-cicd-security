...
describe('verify', () => {
    ...
    describe('jwtChallenges', () => {
        ...
        it('"jwtUnsignedChallenge" is not solved via regularly signed token even with email jwtn3d@juice-sh.op in the payload', () => {
            ...
            verify.jwtChallenges()(req, res, next);
            expect(datacache_1.challenges.jwtUnsignedChallenge.solved).to.equal(false);
        });
        if (!(0, utils_1.isWindows)()) { // The "jwtForgedChallenge" is disabled on Windows due to an incompatibility
